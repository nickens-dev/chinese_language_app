...
const report: ProgressReport = {
  ...
  cards:[{ itemId:"card-1", simplified:"好", pinyin:"hǎo", english:"good", promptChannel:"characters", responseChannel:"english", correct:3, attempts:4, accuracyPercent:75, averageScore:82, lastStudiedAt:"2026-08-10T12:00:00Z" }],
...
test("shows progress aggregates and updates direction filters", async () => {
  ...
  render(<ProgressPage decks={decks} onStudy={vi.fn()}/>);
...
test("starts study from progress", async () => {
  vi.mocked(progressApi.fetchProgress).mockResolvedValue(report);
  const onStudy = vi.fn();
  render(<ProgressPage decks={decks} onStudy={onStudy}/>);
