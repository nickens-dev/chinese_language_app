...
def _pinyin(value: str, keep_tones: bool = True) -> str:
    syllables: list[str] = []
    for token in re.findall(r"[a-zA-ZüÜvV:]+[1-5]?|[^\W\d_]+", value.casefold()):
        tone, letters = "", ""
...
def evaluate(answer: str, channel: StudyChannel, snapshot: dict[str, str]) -> StudyAttemptResult:
    ...
    return StudyAttemptResult(score=round(score, 3), verdict=verdict, expectedAnswers=expected, feedback=feedback, evaluatorVersion=EVALUATOR_VERSION)
