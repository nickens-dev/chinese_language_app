...
export function StudyRunner({ initialSession, onExit }: StudyRunnerProps) {
  ...
  return <section className="page study-runner">
    ...
    <div className="prompt-card"><p className="kicker">{prompt.promptChannel} → {prompt.responseChannel}</p><p className="selection-reason"><strong>Why this card:</strong> {prompt.selectionReason}</p><div className={`study-prompt prompt-${prompt.promptChannel}`} lang={prompt.promptChannel === "characters" ? "zh-Hans" : undefined}>{prompt.promptText}</div>
